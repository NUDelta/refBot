...
def startReflection(channel, user_id, user_name, users_collection):
    if users_collection.find_one({"user_id": user_id}) == None:
        mydict = {"user_id": user_id, "user_name": user_name,
                  "channel_id": channel, 'question': 1}
...
def getQuestion(channel, users_collection):
    if users_collection.find_one({"channel_id": channel}) == None:
        ...
        question = users_collection.find_one(
            {"channel_id": channel})['question']
        return question
...
def storeResponse(channel, user_id, user_name, response, question_collection, users_collection):

    if question_collection.find_one({"user_id": user_id}) == None:
        # first question!
        response = [response]

        mydict = {"user_id": user_id, "user_name": user_name, "channel_id": channel, '1': response,
                  '2': [],
                  '3': [],
                  '4': [],
                  '5': [],
        ...
        question_collection.update({'user_id': user_id}, {
            '$push': {questionNum: response}})


def getActionItem(channel, question_collection):
    response = question_collection.find_one({"channel_id": channel})['6']
    print(response)
